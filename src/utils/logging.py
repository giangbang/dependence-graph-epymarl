...
class Logger:
    ...
    def setup_tb(self, directory_name, unique_token):
    ...
    def setup_vizualize(self, vid_dir):
        self.vid_dir = vid_dir
        os.makedirs(vid_dir, exist_ok=True)

    def setup_csv(self, directory, token):
        """mainly for rliable"""
        self.csv_logger = CSVLogger(directory, token)
...
class CSVLogger:
    ...
    def __init__(self, dir, token: str):
        ...
        print("Create logging dir for CSV files at", self.dir)
        self.res = []
        token = token.replace("//", ".")
        token = token.replace("/", ".")
        token = token.replace("\\", ".")
        self.filename = os.path.join(dir, f"{token}.csv")
        print("CSV file saved at", self.filename)
        self.last_dump = -int(1e9)

    def log(self, total_returns, timestep):
        """Total return contains all the total rewards, for
        example, if we run 100 games, then total returns is a list of length 100"""

        self.res.append([timestep, *total_returns])
        if timestep - self.last_dump > int(1e4):
            self.last_dump = timestep
            self.dump()
