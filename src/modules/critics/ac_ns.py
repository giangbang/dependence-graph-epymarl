...
from modules.critics.mlp import MLP, CNN_MLP
...
class ACCriticNS(nn.Module):
    def __init__(self, scheme, args):
        ...
        self.output_type = "v"
        assert isinstance(input_shape, int), type(input_shape)

        # Set up network layers
        Net_cls = MLP if isinstance(input_shape, int) else CNN_MLP
        self.critics = [
            Net_cls(input_shape, args.hidden_dim, 1) for _ in range(self.n_agents)
        ]
