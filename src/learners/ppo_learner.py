...
class PPOLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        if self.use_gae:
            return self._gae_train(batch, t_env, episode_num)
        else:
            return self._epy_train(batch, t_env, episode_num)

    def train_critic_sequential_gae(
        self, critic, target_critic, batch, rewards, mask, target_returns
    ):
    ...
    def _gae_train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        with th.no_grad():
            ...
            advantages = utils.compute_adv_gae(
                self.args.gamma, rewards, mask, old_vals, 0.95
            )
    ...
    def _epy_train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        """Train code in EpyMARL"""
        # Get the relevant quantities
