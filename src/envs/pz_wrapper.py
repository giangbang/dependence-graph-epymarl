...
class PettingZooWrapper(gym.Env):
    ...
    def __init__(self, lib_name, env_name, pz_lib="pettingzoo", **kwargs):
        env = importlib.import_module(f"{pz_lib}.{lib_name}.{env_name}")
...
# custom envs
import envs.custom_pz as custom_pz
envs = Path(custom_pz.__path__[0]).glob("**/*_v?.py")
for e in envs:
    name = e.stem.replace("_", "-")
    lib = e.parent.stem
    filename = e.stem

    gymkey = f"custom-pz-{lib}-{name}"
    gym.register(
        gymkey,
        entry_point="envs.pz_wrapper:PettingZooWrapper",
        kwargs={
