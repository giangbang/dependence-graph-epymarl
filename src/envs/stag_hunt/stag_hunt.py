# https://github.com/wendelinboehmer/dcg/blob/master/src/envs/stag_hunt.py
'''
...
class StagHunt(MultiAgentEnv):
    ...
    def step(self, actions):
        ...
        reward = (
            np.ones((self.batch_size, self.n_agents), dtype=float_type)
            * self.time_reward
        )
            ...
            for a in np.random.permutation(self.n_agents):
                ...
                if self.agents_not_frozen[a, b] > 0:
                    ...
                    if not (np.random.rand() < self.mountain_slope and actions[a, b] == 3):
                        ...
                        if collide:
                            reward[b, a] += self.collision_reward
                ...
                if self.prey_alive[p, b] > 0:
                    ...
                    if self.capture_action:
                        ...
                        if n_catching_agents > 0 and not captured:
                            for a in catching_agents:
                                reward[b, a] += self.miscapture_punishment
                    ...
                    if captured:
                        ...
                        if len(catching_agents) == 0:
                            rew /= self.n_agents
                            reward[b] += rew
                        else:
                            rew /= len(catching_agents)
                            for a in catching_agents:
                                reward[b, a] += rew
                        ...
                        if self.print_caught_prey:
                            print(
                                "Captured %s at time %u, height %d and pos %u."
                                % (
                                    ("stag" if self.prey_type[p, b] == 1 else "hare"),
                                    self.steps,
                                    self.env_max[0] - 1 - self.prey[p, b, 0],
                                    self.prey[p, b, 1],
                                ),
                                "   Agents: ",
                                self.agents_not_frozen[:, b],
                                "   reward %g" % reward[b].sum(),
                            )
                    else:
        ...
        if self.batch_mode:
            ...
            return self._obs, reward[0], terminated[0], truncated, info
