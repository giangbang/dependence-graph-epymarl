...
class GridworldEnv:
    ...
    def __init__(
        ...
        # in case we lack colors due to too many agents
        if self.n_agents > len(COLOR_LIST):
            num_needed_more = self.n_agents - len(COLOR_LIST)
            for k in range(num_needed_more):
                COLOR_LIST.append(rgb_to_hex(self._choose_random_color(seed=k*23)))
        ...
        self.observation_shape = np.prod(self.grid_map_shape).item() + self.n_agents + len(self._see_goal()) + len(self._see_door())
        print("Obs grid space", self.observation_shape)
    ...
    def _see_goal(self):
        """which goals are still available"""
        goal = np.zeros((self.n_agents, ))
    ...
    def get_obs(self, agent, i):
        obs = np.zeros(self.grid_map_shape)

        # for agent in self.current_agent_list:
        #     obs.append([agent.x, agent.y])
        obs[agent.x, agent.y] = 1

        # self player is always first
        # obs[0], obs[i] = obs[i], obs[0]
        obs = np.array(obs).reshape(-1)
        
        onehot = np.zeros((self.n_agents,))
        onehot[i] = 1

        door = self._see_door()
        goal = self._see_goal()
        res = np.concatenate((obs, onehot, door, goal))
        # res = np.concatenate((obs, door, goal))
        assert res.shape[0] == self.observation_shape, f"{res.shape} {self.observation_shape}"
        return res
    ...
    def step(self, actions):
        ...
        for i, (agent, act) in enumerate(zip(self.current_agent_list, actions)):
            ...
            if (next_step != current_coordinate).any() and can_move_to and current_cell.type == SWITCH and current_cell.target is not None:
            ...
            current_cell = self.get(agent.x, agent.y)
            if current_cell.type == SWITCH and current_cell.target is not None:
        ...
        self.setup_grid_agent()
        obses = []
        for i, agent in enumerate(self.current_agent_list):
            obses.append(self.get_obs(agent, i))
        ...
        if not self.separated_rewards:
            rewards = np.sum(rewards).item()
        return obses, state, rewards, done, infos, avail_actions
    ...
    def reset(self):
        ...
        # avail_actions = self.get_avail_actions()
        avail_actions = None
        return obses, state, avail_actions
    ...
    def _gridmap_to_image(self, graph=None, strength=None):
        import cv2
        ...
        for i in range(self.grid_map.shape[0]):
            for j in range(self.grid_map.shape[1]):
                ...
                if current_cell.type == WALL:
                ...
                elif current_cell.type == EMPTY or current_cell.type == DOOR:
                    img_cells[i, j] = empty_cell_image
                elif current_cell.type == SWITCH:
                    color = self._choose_random_color(i*131 + j)
                    ...
                    if current_cell.target is not None and \
                                not self.get(current_cell.target[0], current_cell.target[1]).open:
        ...
        if graph is not None:
            ...
            for i, a in enumerate(self.current_agent_list):
                for j, b in enumerate(self.current_agent_list):
                    if i == j: continue
                    if graph[i, j]:
                        up = 1 if i > j else -1
                        start_point = np.array((pad_img_cell + a.y * cell_size + cell_size//2, 
                                       pad_img_cell + a.x * cell_size + cell_size//2))
                        end_point = np.array((pad_img_cell + b.y * cell_size + cell_size//2, 
                                       pad_img_cell + b.x * cell_size + cell_size//2))
                        color = hex_to_rgb(default_colors[i])
                        thickness = 2
                        start_point[1] += up * 10
                        end_point[1] += up * 10
                        cv2.arrowedLine(return_image, start_point, end_point, color, thickness)
                        if strength is not None:
                            middle_point = (start_point + end_point)/2
                            middle_point[1] -= 2
                            org = middle_point.astype(int)
                            font = cv2.FONT_HERSHEY_SIMPLEX
                            fontScale = .5
                            thickness = 1
                            lineType = cv2.LINE_AA
                            text = f"{strength[i, j]:.2f}"

                            cv2.putText(return_image, text, org, font, fontScale, color, thickness, lineType)

        return return_image

    def _choose_random_color(self, seed=1):
        ...
        return rng.choice(range(128, 256), size=3)
    ...
    def render(self, graph=None, strength=None, **kwargs):
        img = self._gridmap_to_image(graph, strength)
        return img
...
def hex_to_rgb(hex_color):
    """
    Converts a hexadecimal color string (e.g., "#RRGGBB") to an RGB tuple.
    """
    hex_color = hex_color.lstrip('#')  # Remove '#' if present
    
    # Ensure the hex string has the correct length for R, G, B components
    if len(hex_color) != 6:
...
def rgb_to_hex(rgb_tuple):
    """Converts an RGB tuple (e.g., (255, 165, 0)) to a hexadecimal string (e.g., '#FFA500')."""
...
if __name__ == "__main__":
    ...
    import cv2
    cv2.imwrite(f"render_plan{plan}.png", 
                cv2.cvtColor(env.render(np.ones((n_agent, )*2),
                                        np.random.rand(n_agent, n_agent)), 
                             cv2.COLOR_RGB2BGR))