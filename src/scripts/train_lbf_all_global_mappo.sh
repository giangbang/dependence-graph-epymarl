...
scenarios=( "custom_lbf:Custom-Foraging-8x8-2p-4f-coop-v3" "custom_lbf:Custom-Foraging-10x10-2p-3f-coop-v3" "custom_lbf:Custom-Foraging-15x15-3p-4f-coop-v3" )
scenarios=( "lbforaging:Foraging-15x15-4p-3f-v3" "lbforaging:Foraging-15x15-3p-4f-v3" "lbforaging:Foraging-10x10-3p-3f-v3" )
...
for ((i=0; i<n_exp; i++)); do
    ...
    do
        ...
        python src/main.py --config=${algo} --env-config=gymma with env_args.time_limit=50 \
                ...
                use_gae=True exp_name="global" test_deterministic=False
    done
done