...
# scenarios=( "lbforaging:Foraging-10x10-2p-3f-coop-v3" )
# scenarios=( "custom_lbf:Custom-Foraging-15x15-3p-4f-coop-v3" "custom_lbf:Custom-Foraging-8x8-2p-4f-coop-v3" "custom_lbf:Custom-Foraging-10x10-2p-3f-coop-v3" )
scenarios=( "custom_lbf:Custom-Foraging-10x10-2p-3f-coop-v3" )
...
for seed in `seq ${seed_max}`; do
    for ((i=0; i<n_exp; i++)); do
        ...
        python src/main.py --config=${algo} --env-config=gymma with env_args.time_limit=50 \
                ...
                use_full_path=True use_er_graph=True exp_name="er_graph_p=.7" p=0.7 test_deterministic=False
    done
done