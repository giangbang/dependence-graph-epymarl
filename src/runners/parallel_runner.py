...
class ParallelRunner:
    def __init__(self, args, logger):
        self.args = args
        self.logger = logger
        self.batch_size = self.args.batch_size_run  # Number of environments to run in parallel
        ...
        self.use_knn_graph = getattr(self.args, "use_knn_graph", False)
    ...
    def reset(self):
        ...
        if self.use_knn_graph:
            for parent_conn in self.parent_conns:
                parent_conn.send(("get_knn_graph", self.args.k))

            graph_data = {"graph": []}
            for parent_conn in self.parent_conns:
                data = parent_conn.recv()
                graph_data["graph"].append(data)

            self.batch.update(graph_data, ts=0)
    ...
    def run(self, test_mode=False):
        ...
        while True:
            ...
            actions = self.mac.select_actions(
                ...
                test_mode=test_mode and self.args.test_deterministic,
            )
            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if not terminated[idx]:
                    ...
                    if isinstance(episode_returns[idx], (float, int)):
                        assert isinstance(data["reward"], float), data["reward"]
                    else:
                        assert len(episode_returns[idx]) == len(data["reward"]), data["reward"]
...
def env_worker(remote, env_fn):
    ...
    while True:
        ...
        if cmd == "step":
            ...
            env.save_replay()
        elif cmd == "get_knn_graph":
            k = data
            remote.send(get_knn_graph(env, k=k)[0])
        else:
